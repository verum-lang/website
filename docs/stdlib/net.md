...
|---|---|---|
| `addr.vr`           | **partial**          | [core-tests/net/addr](https://github.com/verum-lang/verum/tree/main/core-tests/net/addr) — Ipv4Addr/Ipv6Addr/IpAddr/SocketAddrV4/SocketAddrV6/SocketAddr full surface + RFC 5735/1918/5771/4291 predicates + parse happy/error + AddrParseError disjointness + **Display rendering**. **Cross-tier `--aot` validated 2026-06-20: Tier-0 139/139, Tier-1 95/138.** The Tier-1 gap = 4 root-caused codegen defect classes (see [`addr/audit.md §3.5`](https://github.com/verum-lang/verum/tree/main/core-tests/net/addr/audit.md)): **TUPLE-EQ-AOT** (tuple `==` always true → `is_unspecified`/`is_broadcast`), **DISP-EMPTY-AOT** (f-string Display of user types → empty), **PRELUDE-FREEFN** (`f"{x:?}"`→`format_debug` unbound under AOT/run), **PARSE-AOT** (v4/v6/socket parse). Construction, scalar predicates, `to_u32`/`from_u32`, accessors, `AddrParseError` Eq pass both tiers. Fixed test bug: `fe90` **is** link-local (`fe80::/10` = `fe80..=febf`). |
