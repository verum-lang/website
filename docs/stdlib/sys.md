...
status: partial
status_detail: "Interp 2026-06-11 (post Bug A+B): the covered subset leaf modules fully green; Class 1 (a landed fix) lifted darwin/tls+darwin/io to ~the covered subset. AOT (2026-06-13): the old §D 'module not found' suite-wide block is RESOLVED — stdlib mounts compile. AOT now fails on STRICT type errors the lenient interp masks; two fundamental classes fixed: UInt mis-registered as Type::Generic (`as UInt` → E401) and qualified variant construction `T.Variant(args)` under generic signatures (→ E400), taking cabi --aot 6→the covered subset. Remaining AOT failures: runtime List.iter SIGSEGV (AOT-ITER-1), and Bug C — umbrella re-export under archive load drops a type's impl block (`mount core.sys.{MemProt}` → MemProt.NONE infers Unit; direct `core.sys.common.{MemProt}` works). Full analysis in core-tests/sys/SYS_SPECTRUM_AUDIT.md."
---
...
|---|---|---|
| `common.vr`        | **partial** | [core-tests/sys/common](https://github.com/verum-lang/verum/tree/main/core-tests/sys/common) — the type-level surface is covered: `PAGE_SIZE`, the page-alignment helpers, `OSError`, `FileDesc`, `IOVec`, `MemProt`, `MapFlags`, `SysContextError`, `FcntlLockKind` and the `ACCESS_*` / `SEEK_*` / `F_*LCK` constant families, plus the context-slot limits. **Known limitation:** a selective re-export of a constant — `mount core.sys.{PAGE_SIZE}` — does not register the name for code generation. Mount the module and qualify: `mount core.sys; sys.PAGE_SIZE`. |
...
| **`darwin/mod`** | **complete** | [core-tests/sys/darwin/mod](https://github.com/verum-lang/verum/tree/main/core-tests/sys/darwin/mod) — 23u + 11p + 7i + 5r. Umbrella path-equivalence: every umbrella ↔ direct submodule value preserved across libsystem/mach/errno/tls/io/thread/time. |
