...
status: partial
status_detail: 11 complete (bitfield / cabi / init / interrupt / no_runtime / mod-umbrella / darwin-errno / darwin-mod / linux-errno / linux-mem / linux-auxv), 26 partial (~14 sys submodules + 8 darwin/* + 8 linux/* platform modules), 1 regression-only (signal — variant-tag drift). See per-submodule rows in `core-tests/INVENTORY.md`.
---
