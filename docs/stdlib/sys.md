...
status: partial
status_detail: 4 complete (bitfield / cabi / init / interrupt — see closures below), ~10 partial (common / mmio / context_ops / file_ops / process_ops / net_ops / durability / fs_watch / io_engine / locking / time_ops), 1 regression-only (signal — variant-tag drift). See per-submodule rows in `core-tests/INVENTORY.md`.
---
