...
> cross-module free-function dispatch defect was closed by task #121
> (see
...
| # | Defect | Status |
|---|---|---|
| 1 | Cross-module free-function dispatch silently returns `Unit/nil` at `--interp` runtime — affects every `mount X.{free_fn}` and `module.fn(args)` call site (workspace-wide; `core.base.glob.matches` exhibits the same shape). | **closed** in task #121 |
| 2 | `mount X.{public_const}` does not register the name in the codegen's global symbol table; cross-module imports of `public const` items report `UndefinedVariable("CONST_NAME")` at codegen. Workaround: `mount X; X.CONST`. | partial — bare-mount form `mount X.Y.Z; Z.CONST` closed; selective `mount X.{CONST}` form still falls through (task #15) |
