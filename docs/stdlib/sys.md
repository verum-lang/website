...
| **`linux/mod`** | **partial** | [core-tests/sys/linux/mod](https://github.com/verum-lang/verum/tree/main/core-tests/sys/linux/mod) — 7 unit umbrella reachability tests for errno + mem + time + tls constants. Full path-equivalence sweep deferred. |
...
| **`linux/bpf/map`** | **partial** | [core-tests/sys/linux/bpf/map](https://github.com/verum-lang/verum/tree/main/core-tests/sys/linux/bpf/map) — 20u. MapType ADT (30 variants matching `enum bpf_map_type` in `<linux/bpf.h>` UAPI — Hash/Array/PerfEventArray/PercpuHash/LpmTrie/Devmap/Sockmap/Xskmap/Queue/Stack/RingBuf/BloomFilter/…); MapConfig record round-trip including BPF_F_MMAPABLE flag use case; MapDescriptor field preservation across record construction. bpf() syscall round-trip (create_map / map_lookup / map_update / map_delete / map_iterate) deferred to Linux runner. |
| **`linux/bpf/program`** | **partial** | [core-tests/sys/linux/bpf/program](https://github.com/verum-lang/verum/tree/main/core-tests/sys/linux/bpf/program) — 22u. ProgramType ADT (13 of 32 variants — Unspec / SocketFilter / Kprobe / Xdp / Tracepoint / SchedCls / SchedAct / CgroupSkb / RawTracepoint / StructOps / Lsm / SkLookup / Syscall); AttachType (9 of 39 variants — CgroupInetIngress / CgroupInetEgress / TraceFentry / TraceFexit / XdpDevmap / XdpCpumap / PerfEvent / ModifyReturn / LsmMac); ProgramBytecode / ProgramDescriptor / Link record-shape round-trip with `link.fd != link.program_fd` invariant + `GPL` and `dual BSD/GPL` license-string acceptance. load_program + attach_* deferred to Linux runner. |
| **`linux/bpf/mod`** | **partial** | [core-tests/sys/linux/bpf/mod](https://github.com/verum-lang/verum/tree/main/core-tests/sys/linux/bpf/mod) — 9 unit umbrella reachability tests for BpfError + ProgramType + AttachType + ProgramDescriptor + ProgramBytecode + Link + MapType + MapDescriptor + MapConfig. Function re-exports (load_program / attach_* / create_map / map_*) deferred to Linux runner. |
...
> green under `--interp`; AOT path is gated by unrelated stdlib
> bare-variant defects (see "Open defects" below).
