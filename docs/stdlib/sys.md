...
status: partial
status_detail: "Interp 2026-06-11 (post Bug A+B): 39/51 leaf modules fully green; Class 1 (487cef871) lifted darwin/tls+darwin/io to ~41/51. AOT (2026-06-13): the old §D 'module not found' suite-wide block is RESOLVED — stdlib mounts compile. AOT now fails on STRICT type errors the lenient interp masks; two fundamental classes fixed: UInt mis-registered as Type::Generic (`as UInt` → E401) and qualified variant construction `T.Variant(args)` under generic signatures (→ E400), taking cabi --aot 6→35/50. Remaining AOT failures: runtime List.iter SIGSEGV (AOT-ITER-1), and Bug C — umbrella re-export under archive load drops a type's impl block (`mount core.sys.{MemProt}` → MemProt.NONE infers Unit; direct `core.sys.common.{MemProt}` works). Full analysis in core-tests/sys/SYS_SPECTRUM_AUDIT.md."
---
