...
status: partial
status_detail: 22 modules; 15 audited via core-tests/async/; 8 fundamental defects pinned (tasks #6 – #17 + #6 CLOSED 2026-05-24, #8 pinned). The variant-algebra + construction-surface backbone + stream factory + protocol-mountability surface are interpreter-green; runtime poll-path coverage waits on the executor test-bed (task #8 close).
---
...
<StdlibStatus
  status="partial"
  detail="15 of 22 async modules carry full conformance suites; 8 fundamental compiler/stdlib defects pinned. Variant-algebra + construction-surface backbone + stream factory + protocol-mountability surface are interpreter-green; runtime poll-path coverage waits on the executor test-bed (task #8 close)."
  defects={[
    ...
    {area: 'stream', summary: '**CLOSED 2026-05-24:** task #6 (`Maybe.take()` in-place mutation aliasing) — 3-part fix in `dispatch_variant_method`: (1) clone variant payload to fresh heap allocation BEFORE clearing original; (2) `INTRINSIC_PREFERRED_VARIANT_METHODS` short-circuit forces intrinsic dispatch over user-side body; (3) variant type-id gate extended to accept `TypeId::MAYBE`/`RESULT` in addition to synthetic ids.  Closes the entire `&mut self → self.value.take()` class for stream_once, stream_repeat_n, stream_unfold, every option-shaped state machine. Open: task #7 (`T.clone()` codegen intercept on generic receiver) — pins StreamRepeatN with non-Copy payload.'},
    {area: 'executor', summary: 'task #8 — FFI symbol intercept lookup gap; `verum_get_runtime_*` extern intercept in `ffi_extended.rs:1639` reads empty getter-name from `FfiSymbolId(1)`, falls through to FFI runtime which fails. Blocks every `RuntimeBuilder.new()` / `AsyncRuntimeConfig.default()` chain under --interp.'},
...
| `executor.vr`      | **regression-only** | [core-tests/async/executor](https://github.com/verum-lang/verum/tree/main/core-tests/async/executor) — 25 contract-surface tests (RuntimeBuilder.new/setters/full-chain + AsyncRuntimeConfig.default/cpu_bound/io_bound presets + AsyncRuntimeConfig setter surface + List<RuntimeBuilder> bulk container + RuntimeConfig alias + 6 algebraic laws covering setter commutativity / construction independence / preset non-interference / idempotent default). All 25 actively `@ignore`'d pending **task #8** (FFI symbol intercept lookup gap — `verum_get_runtime_*` extern intercept reads empty getter-name from `FfiSymbolId(1)`, falls through to FFI runtime which fails). Test suite stays as the canonical contract surface; flips back to active the moment #8 lookup resolves. |
| `stream.vr`        | **partial**  | [core-tests/async/stream](https://github.com/verum-lang/verum/tree/main/core-tests/async/stream) — 7 unit + 8 property + 4 integration + 6 regression GREEN under interpreter; 2 `@ignore`d for task #7 (`T.clone()` codegen intercept on generic receiver — affects `StreamRepeatN.poll_next`). Pinned: size_hint laws (empty/once/repeat_n boundaries), poll_loop exhaustion stickiness, generator-driven termination via stream_from_fn, List drain via manual noop-context poll driver, stream_once → Maybe → Poll integration. **Task #6 CLOSED 2026-05-24** via 3-part fundamental fix in `dispatch_variant_method` — clone-before-mutate semantics for `Maybe.take()` + intrinsic-preferred dispatch + well-known TypeId variant gate. Combinator-chain coverage (StreamMap/StreamFilter/StreamTake) deferred behind task #22 (constrained-implement-block dispatch). |
