...
| `barrier.vr`   | **partial** | [core-tests/sync/barrier](https://github.com/verum-lang/verum/tree/main/core-tests/sync/barrier) — 21 unit + 8 property + 8 regression, of which 15 @ignore'd (audit § 3.1 secondary — same Tier-0 futex FFI symbol gap; Phaser.register/terminate and CountDownLatch.count_down-to-zero touch futex_wait/wake).  Live path (Barrier.new + accessors + BarrierWaitResult shape + Default + Phaser.new + atomic accessors + CountDownLatch.new + zero-counter drained-immediate fast-path) all GREEN.  Live multi-thread rendezvous + futex paths at L2. |
