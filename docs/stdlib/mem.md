...
| `thin_ref.vr` | <LifecycleBadge lifecycle="theorem" version="v0.1" /> | <TierBadge tier="interp" /> | <TestCovBadge cov="full" /> | `core-tests/mem/thin_ref/` — 4 files + audit (static-shape only). 17/17 unit + `UseAfterFreeError` 4/6 record-literal tests GREEN under `--interp` (record-literal construction path baseline). 2 `.new(...)` static-method tests remain `@ignore`'d on the cross-module record-return type-name propagation defect (audit §6); a defensive codegen field-resolver fallback was attempted (commit `ab8e707f4`) but introduced field-write OOB regressions on previously-GREEN tests and was reverted (commit `585728904`, audit §7) — the correct fundamental fix must preserve the 4-way cache-consistency invariant `(type_name_to_id, self.types, type_field_layouts, type_field_type_names)` simultaneously. |
