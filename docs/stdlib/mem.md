...
| `thin_ref.vr` | <LifecycleBadge lifecycle="theorem" version="v0.1" /> | <TierBadge tier="interp" /> | <TestCovBadge cov="full" /> | `core-tests/mem/thin_ref/` — 4 files + audit (static-shape only). 17/17 unit + `UseAfterFreeError` 4/6 record-literal tests GREEN under `--interp` via codegen `ab8e707f4` defensive `self.types`-by-name field-resolver fallback; `.new(...)` static-method 2 tests remain `@ignore`'d on the cross-module record-return type-name propagation defect (audit §6). |
