...
| `trie.vr`           | **regression-only** | [core-tests/collections/trie](https://github.com/verum-lang/verum/tree/main/core-tests/collections/trie) — 3 unit + 3 property + 2 integration + 6 regressions. Trie mutator path (insert/remove) gated on `Map.get_mut` dispatch (same wrapper-iter class as multiset §B / slice §D). Empty-state surface green. |
