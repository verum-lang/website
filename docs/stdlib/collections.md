...
|---|---|---|
| `list.vr`           | **partial** | [core-tests/collections/list](https://github.com/verum-lang/verum/tree/main/core-tests/collections/list) — 136 unit (107 active + 29 @ignore) + 11 property + 8 integration + 9 pinned regressions, **108/109 active green** on `--interp` validated 2026-05-27. **Round 12 (2026-05-27)**: bi-modal `position` runtime intercept (closure vs value-form per `arg0.is_func_ref()`); stdlib migration of 8 ptr-deref methods (get_mut / pop_front / split_off / binary_search / binary_search_by / partition_point / try_push / append) to runtime-intercepted `self.get(i)` / `self.remove(i)` / `self.truncate(at)` surface; `position` / `rposition` migrated from `for i in range(...)` to `while + self.get(i)` (commits 2b953400b + 41b379631 + 0c2ae35a1 + e1c6bddf6 + d097d2ba0). Defect classes pinned @ignore: §E for/range iterator-closure remap (find_by / position_by / starts_with / ends_with / windows / chunks / partition_by / chunk_by / dedup_by); resize_buffer SIGABRT on `realloc(self.ptr, ...)` (reserve / shrink); cross-module bare-name (try_with_capacity); introsort closure dispatch (sort_unstable populated / sorted). **List.from / List.of / List.from_elem CLOSED 2026-05-24** (commit 0360a343e): `List.from([1,2,3])` was silently returning empty list because `for item in [T; N]` no-op'd in codegen; rewritten to index-based iteration. Three regression tests un-ignored and green. 4 runtime intercepts added (contains-needle CBGR deref, set, get_or, swap_remove, truncate); layout-drift defects pinned. |
| `map.vr`            | **partial** | [core-tests/collections/map](https://github.com/verum-lang/verum/tree/main/core-tests/collections/map) — 34 unit (33 active + 1 @ignore) + 6 property + 5 integration + 5 pinned regressions, **33/34 active green** on `--interp` validated 2026-05-27. **Round 12 expansion**: Sections 10–14 added (get_or / with_capacity / insert overwrite contract / remove return contract / many-keys preserved under growth + half-remove). Map.with_capacity(N) then N inserts triggers a resize-at-load-factor defect (1 @ignore'd). Cap=0 bootstrap guard added on insert; remove / contains_key / get now empty-map-safe. |
...
| `heap.vr`           | **partial** | [core-tests/collections/heap](https://github.com/verum-lang/verum/tree/main/core-tests/collections/heap) — 25 unit (23 active + 2 @ignore) + 5 property + 4 integration + 5 regressions, **23/25 active green** on `--interp` validated 2026-05-27. **Round 12 expansion**: Sections 6–11 added (with_capacity / push_pop (3 variants) / replace (2 variants) / try_push / singleton / inserted-in-both-orders / duplicates). Two new @ignore'd defects: `replace(v)` on populated heap dispatches `List.swap` to receiver mis-classified as `InterruptCell (Int)` (receiver-type loss between CallM site and dispatch table); `push_pop(v < max)` returns wrong value. Earlier branch: Heap.into_inner CBGR runtime intercept (commit `bd2553407`); array-dispatch polarity defect closed. |
...
| `slice.vr`          | **partial** | [core-tests/collections/slice](https://github.com/verum-lang/verum/tree/main/core-tests/collections/slice) — 32 unit + 10 property + 5 integration + 17 regressions (5 PASS-GUARDs + 12 defect-pins). **index_of/rindex_of CLOSED 2026-05-24** (commit d05ffd479): missing `public` qualifier prevented user-side dispatch. 7/14 slice regressions green. Working surface: len/is_empty/first/last/get/slice(a,b)/split_at/min/max/contains/iter/index_of/rindex_of. Slice-only methods (eq_slice/cmp_slice/to_list/binary_search/partition_point/chunks/windows) still unreachable via CallM because `&[T]` shares the `List` runtime kind. |
