...
| **L4 VDBE / L5 SQL end-to-end** | **VBC codegen now deterministic; remaining blocker is `Iterator.clone` dispatch** | The historical "method 'X' not found / Null pointer / field-OOB / division-by-zero — different panic each run for the same source" symptom matrix was caused by HashMap iteration leaking Rust's per-process random hasher seed into VBC FunctionId/TypeId assignment.  Fixed across commits 0723ad43 (`pipeline.rs`) + 82303f94 (`codegen/mod.rs` + `phases/vbc_codegen.rs`) — 7 sort-by-stable-key sites total.  Verified: 4 consecutive runs of an L7 `CREATE TABLE + INSERT + SELECT` probe now produce byte-identical output.  Locked in via `crates/verum_compiler/tests/vbc_codegen_determinism.rs` (two-process determinism contract).  The remaining stable panic ("method 'FilterMapIter.clone' not found on value") is a coherent stdlib `Iterator.clone` gap — addressable on its own, not a hash-seed roulette |
