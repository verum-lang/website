...
| `provider.vr`  | **partial** | [core-tests/context/provider](https://github.com/verum-lang/verum/tree/main/core-tests/context/provider) — `Provider<T>` lazy factory + `get_context`/`has_context`, with Text/List/Maybe/tuple/`ContextLogLevel` carriers, all green under `--interp`. `has_context` is checked at both polarities in one programme: `false` for a slot nothing provided, `true` inside an active provide, `false` again after the end — a predicate that answered a constant would pass a one-sided check. Not yet green: `ScopedProvider.run` and its slot-lifecycle regression, which fail on defects of their own (a null receiver inside `Sender.send`, and a `lock` method that does not resolve on an `Int` receiver) rather than on the context store. Lazy `map`/`flat_map` over env-capturing closures, the `Result` overload of `get_context`, and `@bitcast` payload sizing remain deferred. |
