...
| `cron.vr`           | **partial** | [core-tests/time/cron](https://github.com/verum-lang/verum/tree/main/core-tests/time/cron) — the full suite passes under the interpreter, including the floor semantics of `decompose` and minute alignment for instants before the epoch. **Known limitation:** compiled ahead of time, a cron field written as a bare number is rejected — alias resolution reads a slice whose length it cannot see correctly. Wildcards and step expressions parse. Use the interpreter for cron schedules meanwhile. |
