...
| `duration_parse.vr` | **partial** | [core-tests/time/duration_parse](https://github.com/verum-lang/verum/tree/main/core-tests/time/duration_parse) — interp 60/60 GREEN. AOT 58/60 (2026-07-10, was 9/60 → 2/60 → 58/60 across the as_slice, name-coherence and uniform-i64 rounds; `parse("30s")` correct under AOT). The 2 residuals are error-path tests failing only inside the test pipeline — exact standalone repros pass both tiers (task #21). |
...
| `cron.vr`           | **partial** | [core-tests/time/cron](https://github.com/verum-lang/verum/tree/main/core-tests/time/cron) — interp 42/42 GREEN. Same floor-semantics class as rfc3339 fixed in `decompose` + the `next_after_unix` minute alignment (`next_after_unix(-100)` returned 0, not -60); pre-epoch schedule evaluation pinned (`regression_test.vr §E`). AOT 16/42 (2026-07-10): wildcard/step exprs parse correctly; every bare numeric field returns Err because `resolve_value` iterates a `&[AliasEntry]` slice whose len/layout is broken on BOTH tiers (`&[]` reads len 4 at interp; AOT SIGSEGVs on the probe) — task #19, shared root with rfc3339. |
