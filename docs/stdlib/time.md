...
status: partial
status_detail: >-
  2026-07-09 full re-audit: all 9 submodules (mod.vr's `Time` namespace suite added — mirror closure) GREEN under `--interp` (442 tests) except 5 pins of two open cross-cutting defects (List.sort-on-records raw-Value compare; test-pipeline `Duration / Int` misroute). §G dual-representation CLOSED — the Duration/Instant raw-Int intrinsic alias surface was deleted; the Verum record bodies are the single implementation on both tiers. Signed-Duration arithmetic coherence (checked_add/sub, carry normalisation, signed timeline), pre-1970 floor semantics (rfc3339/cron), ONE canonical monotonic+realtime clock source, and the Time.sleep Tier-0 no-op/negative-hang are all fixed and regression-pinned. AOT parity campaign IN FLIGHT: two systemic roots fixed (as_slice byte-stride over Value-boxed buffers; unresolved-CallM const-zero → runtime type-switch dispatch); per-module AOT status below reflects the pre-fix baseline until the re-run lands.
---
