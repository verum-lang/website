...
status: partial
status_detail: >-
  2026-07-09 full re-audit: all 9 submodules (mod.vr's `Time` namespace suite added — mirror closure) GREEN under `--interp` (442 tests) except 5 pins of two open cross-cutting defects (List.sort-on-records raw-Value compare; test-pipeline `Duration / Int` misroute). §G dual-representation CLOSED — the Duration/Instant raw-Int intrinsic alias surface was deleted; the Verum record bodies are the single implementation on both tiers. Signed-Duration arithmetic coherence (checked_add/sub, carry normalisation, signed timeline), pre-1970 floor semantics (rfc3339/cron), ONE canonical monotonic+realtime clock source, and the Time.sleep Tier-0 no-op/negative-hang are all fixed and regression-pinned. AOT parity campaign IN FLIGHT: two systemic roots fixed (as_slice byte-stride over Value-boxed buffers; unresolved-CallM const-zero → runtime type-switch dispatch); current AOT legs: duration 84/107, system_time 37/56, julian 43/49, rfc3339 34/52, interval 21/33, mod 25/40, instant 20/33, duration_parse 9/60, cron 4/42. The remaining dominant blocker is characterised end-to-end (ptr-era register-slot orphaning across basic blocks — every expression-bodied Maybe→Result mapper returns the wrong variant at AOT; 12-line repro + proven fix preserved on fix/uniform-i64-slot-wip, re-landing gated on the register-load/call-arg coercion audit; task #16).
---
...
| `duration_parse.vr` | **partial** | [core-tests/time/duration_parse](https://github.com/verum-lang/verum/tree/main/core-tests/time/duration_parse) — interp 60/60 GREEN. AOT baseline 9/60 — root-caused to AS-SLICE-BYTE-STRIDE-1 (`trim()` builds a `List<Byte>`; the AOT `as_slice` path marked the register byte-stride over the Value-boxed buffer, so every byte past index 0 read 0 and the parser saw `UnknownUnit("")`); fix landed, re-run pending. |
...
| `cron.vr`           | **partial** | [core-tests/time/cron](https://github.com/verum-lang/verum/tree/main/core-tests/time/cron) — interp 42/42 GREEN. Same floor-semantics class as rfc3339 fixed in `decompose` + the `next_after_unix` minute alignment (`next_after_unix(-100)` returned 0, not -60); pre-epoch schedule evaluation pinned (`regression_test.vr §E`). AOT baseline 5/42 (same as_slice root as duration_parse); re-run pending. |
