...
status_detail: Round-16 conformance landed 2026-05-27. 17 submodules tested under core-tests/runtime/ (295 tests; ~250 GREEN; ~45 @ignore'd on surfaced defects). 8 NEW architectural defect classes documented across audit.md files — primary cluster is `verum.runtime.*` intrinsic stub family (cbgr_* / num_cpus / tls_slot_* / pool_* / text_parse_* / char_is_*) not registered in `crates/verum_vbc/src/interpreter/dispatch_table/handlers/`. Cross-tier --aot validation gated on task #7 (AOT stdlib build cascade).
---
...
<StdlibStatus
  ...
  defects={[
    {area: 'env', summary: '§A EnvTaskId.main() static-method dispatches to wider-record sibling (task #17/#39 root, 3 @ignore pins). Otherwise complete: 23 unit tests on ExecutionTier 4-variant + .overhead_ns CBGR cost contract (15/8/3/0 ns) + monotonicity invariant + .requires_generation / .requires_epoch tier predicates + EnvIsolationLevel + CpuAffinity.'},
    ...
    {area: 'config', summary: '§A no `@cfg_must_be_exclusive(runtime)` annotation; §B NoopDriver magic `Other(-1)` sentinel (replace with NoopDriverInert); §C `AlreadyInitialized` race not atomic (CAS recommended); §D Eq on RuntimeIoError.Other deliberately payload-sensitive. 30/30 GREEN.'},
