...
| `result.vr`          | **partial** | [core-tests/base/result](https://github.com/verum-lang/verum/tree/main/core-tests/base/result) — unit, property, integration, try-block and try-protocol suites all green under the interpreter. **Known limitation:** the `?` operator does not cross between `Maybe` and `Result` — `Maybe<T>?` inside a function returning `Result<T, E>` (and the inverse) is refused, because residual conversion matches structurally and does not consult the `E: Default` bound on the impl. Convert explicitly with `.ok_or(...)` or `.ok()`. |
...
| `ops.vr`             | **partial** | [core-tests/base/ops](https://github.com/verum-lang/verum/tree/main/core-tests/base/ops) — unit, property, integration and regression suites green under the interpreter, covering the whole `ControlFlow<B, C>` / `Try` / `FromResidual` / `Never` / `Drop` surface. **Known limitation:** a chain of nested `if`/`else` returning `Result` can produce the wrong variant, because the return type is not tracked through the chain. Use the `?` operator rather than hand-unrolling it. |
| `protocols.vr`       | **partial** | [core-tests/base/protocols](https://github.com/verum-lang/verum/tree/main/core-tests/base/protocols) — unit, property, integration and protocol-agnostic suites green under the interpreter, covering the operator and capability protocols: `Eq`, `Ord`, `Hash`, `Clone`, `Copy`, `Default`, `Debug`, `Display`, `From`, `Into` and `Add` through `Shr`. **Known limitation:** a short type name can collide with a same-named type elsewhere in the library and resolve to the wrong one — a local `type Item is { id: Int }` collides with the unit type `core.meta.contexts.Item`. Give types distinctive names until name resolution is qualified. |
| `iterator.vr`        | **partial** | [core-tests/base/iterator](https://github.com/verum-lang/verum/tree/main/core-tests/base/iterator) — the basic, property, integration, protocol-agnostic and regression suites are green under the interpreter; the unit suite has a documented tail of deferred cases. **Deferred:** the transducer methods are incomplete, `Range.reduce_with` does not reach its protocol default, `try_fold` loses its result generic, and a closure passed into a method chain does not always receive its expected type. |
| `cell.vr`            | **partial** | [core-tests/base/cell](https://github.com/verum-lang/verum/tree/main/core-tests/base/cell) — the `Cell` family is stable and `Ref` / `RefMut` auto-deref through `RefCell` works. **Known limitation:** `LazyCell.force` requires `RefMut` to have been loaded transitively, so it can fail depending on what else the program mounts. |
...
| `primitives.vr`      | **partial** | [core-tests/base/primitives](https://github.com/verum-lang/verum/tree/main/core-tests/base/primitives) — `Int.checked_add` / `checked_sub`, the `wrapping_*` and `saturating_*` families and `to_le_bytes` are stable, as are the constants — `Int.MAX`, `Int.BITS`, `Float.BITS`, `Float.NAN`, `Float.INFINITY`, `Float.PI`, `Float.MAX`, `Float.MIN`, `Float.EPSILON`, `Float.NEG_INFINITY`. A method called on a cast receiver, `(n as Byte).count_ones()`, keeps the cast type's semantics rather than falling back to `Int`. **Deferred:** width propagation through `saturating_add` on a sized integer, and folding of a `let`-bound cast. |
...
| `semver.vr`          | **partial** | [core-tests/base/semver](https://github.com/verum-lang/verum/tree/main/core-tests/base/semver) — unit, property, integration and regression suites green under the interpreter; the data surface — the five-field `SemVer` record, ordering per the specification's precedence rules, and the four-variant `SemVerError` — is fully covered. **Known limitations:** `parse` can dispatch to a same-named sibling and return the wrong type, and `format` fails on a private-visibility path. Build a `SemVer` with a record literal until both land. |
| `semver_constraint.vr` | **partial** | [core-tests/base/semver_constraint](https://github.com/verum-lang/verum/tree/main/core-tests/base/semver_constraint) — unit, property, integration and regression suites green under the interpreter. The whole constraint algebra is covered: the nine-variant `SemVerConstraint` (`Any`, `Empty`, `Eq`, `Gt`, `Gte`, `Lt`, `Lte`, `And`, `Or`), the convenience constructors (`exact`, `caret`, `tilde`, `tilde_major`, `tilde_minor`, `x_range_major`, `x_range_minor`), flatten-and-absorb composition through `and` / `or`, and `MatchOptions` in strict and allow-prerelease modes. **Known limitation:** `parse` shares the `SemVer.parse` dispatch defect and can return an unrelated type, which then faults downstream. Construct constraints with the convenience constructors instead. |
| `retry.vr`           | **partial** | [core-tests/base/retry](https://github.com/verum-lang/verum/tree/main/core-tests/base/retry) — unit, property, integration and regression suites green under the interpreter, covering the three-variant `RetryBackoff`, the `RetryOptions<E>` builder chain (`.simple`, `.exponential`, `.linear`, `.with_should_retry`) and first-attempt success through every entry point. **Known limitation:** a retry closure that mutates a counter in the enclosing scope fails at run time — closure monomorphisation loses the captured variable's type and the call resolves against an unrelated method. Keep retry closures free of captured mutable state. |
