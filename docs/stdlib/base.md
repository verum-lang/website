...
status: partial
status_detail: 2026-06-01 first comprehensive cross-tier sweep (enabled by the per-file test-compile cache, commit 993679a01 — ordering 7min→29s) measured 904 ok / 139 failed under `--interp` across the 5 largest submodules (ordering 138/5, maybe 157/17, result 205/25, primitives 257/75, protocols 147/17). Fixes landed this cycle: bare + qualified variant Display in f-strings (bf3364e0c + 19bb51b3a, +12 tests), char_to_text ABI (faca36b33), ProcessExit AOT decode (8d1b5cef1), Int.MIN const-emission hardening. AOT suite now feasible (≈1.3s/test, parallel-safe). Open defect clusters (root-caused; see core-tests audits): float edge cases ~50 (NaN/∞/to_bits/precision — basic ops work), try_block/`?` ~15, generic-protocol dispatch ~20 (Debug/fmt_debug/T.default()/Hash on generic &T), Int.MIN ~11 (pre-codegen const value), field-chain ~3. Earlier "complete" claims predate the comprehensive test surface (e.g. primitives float/arithmetic-edge tests added since). See per-module status table below.
---
