...
| `ordering.vr`        | **partial** | [core-tests/base/ordering](https://github.com/verum-lang/verum/tree/main/core-tests/base/ordering) — 28/29 pass under `--interp` + `--aot`. Tier-0 dispatcher inlines `Int.cmp`, `Float.cmp` (NaN-as-Equal), `Bool.cmp` (false &lt; true), `Text.cmp`. **Task #8 CLOSED via commit `d4f65f0c4` (2026-05-14)**: every primitive-method intercept (`cmp`/`eq`/`ne`/`lt`/`le`/`gt`/`ge` on Int/Bool/Float/Byte/Char/Int32/UInt64) now derefs heap-interior refs produced by `RefListElement` (`&xs[i]`) / `RefField` (`&record.f`) via canonical `resolve_arg_value` helper in `cbgr_helpers.rs`, replacing 24+ duplicated `is_cbgr_ref / else .as_T()` sites. Pre-fix `xs[0].cmp(&xs[1])` for `xs = [5, 2]` returned `Less` instead of `Greater` (read pointer-as-i64); every bubble-sort over indexed Int lists was silently a no-op. Remaining 1 failure: `integration_display_ordering_shows_operator_glyph` — `f"{Less}"` returns `Less` instead of `<` because the runtime `ToString` opcode dispatches to a Debug-style variant-name formatter and does NOT invoke the user-defined `Display::fmt` impl (tracked as task #9, a Display-vs-Debug routing architectural defect — separate from #8). |
