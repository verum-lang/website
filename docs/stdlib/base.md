...
status: partial
status_detail: All 27 core.base.* submodules now have full conformance test suites with audit.md + regression_test.vr — 7 complete (maybe / result / cell / coercion / error / uuid / primitives 60/60), 20 partial under `--interp` (every previously undocumented / regression-only / partial-with-FAIL module elevated during the 2026-05-26 / 2026-05-27 base sweep — 16 modules total: coinductive, string_distance, snowflake, ulid, nanoid, semver, retry, semver_constraint, glob, serde, log, mod, panic, protocols, env, ops, data — sweep close commit b2ceb3d88), 0 regression-only, 0 undocumented. See per-module status table below for the truth-table over each `core.base.*` submodule.
---
