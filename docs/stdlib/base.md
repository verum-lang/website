...
status: partial
status_detail: 7 complete (maybe / result / cell / coercion / error / uuid / primitives 60/60), 24 partial (incl. coinductive 4/4 + string_distance 4/4 + snowflake 4/4 + ulid 4/4 + nanoid 4/4 + semver 4/4 — 2026-05-27 sweep), 1 regression-only (retry), 1 undocumented (semver_constraint). See per-module status table below for the truth-table over each `core.base.*` submodule.
---
...
| `retry.vr`           | **regression-only** | [core-tests/base/retry](https://github.com/verum-lang/verum/tree/main/core-tests/base/retry) — 3/14. Closure receiver lowered as non-pointer at the `call_closure` site; `RetryStrategy` builder API stable, `retry()` itself blocked. |
