...
status: partial
status_detail: >-
  2026-06-02 full sweep of ALL 27 base submodules under `--interp` against an isolated, freshly re-baked stdlib archive (enabled by the per-target archive-isolation fix a landed fix — prior sweeps were masked by concurrent builds clobbering the shared `target/precompiled-stdlib/runtime.vbca`): a majority-green sweep with pinned residuals. Clean submodules: nanoid, semver_constraint, serde, snowflake, string_distance, coinductive. META-FINDING (isolated probes): the per-feature codegen LOGIC is correct — Int.MIN, Text.new, and boxed >2^48 bit-ops all work in USER code; base failures are predominantly stdlib-BAKE / cross-module-resolution / archive-plumbing defects (a tracked toolchain task class), NOT codegen bugs. Example: the uuid 22-fail cluster is `Uuid.to_text` baked to a panic-stub because `core.text.Text.new` was an unresolved forward reference when `core.base.uuid` (a lower layer) was precompiled. Fix layers by leverage: (1) a tracked toolchain task cross-module bake forward-refs — large tracked architectural work, must NOT be rushed (naive stub synthesis explodes the archive 12.9→110MB+); (2) thread the `__const_val_` const marker through the test-compile core_metadata load path (Int.MIN ~an extensive test set); (3) the 4 crashers (env isolated to unit_test.vr, likely overlaps the unmerged runtime-codegen-fixes branch); (4) float edge cases ~50. Full per-cluster root causes: `core-tests/base/BASE_SPECTRUM_AUDIT.md`. Fixes landed this cycle: per-target archive isolation (a landed fix), qualified-const consumption LoadK (a landed fix), bare + qualified variant Display (a landed fix + a landed fix), char_to_text ABI (a landed fix), ProcessExit AOT decode (a landed fix). See per-module status table below.
---
...
|---|---|---|
| `maybe.vr`           | **stable** | [core-tests/base/maybe](https://github.com/verum-lang/verum/tree/main/core-tests/base/maybe) — **task #4 CLOSED 2026-05-15**: 3/3 test files green under interpreter (unit / property / integration). `Maybe<T>` Deref impl + bidirectional bare-`None` resolution via task #5 §3.1 close the typecheck-time surface. |
| `result.vr`          | **partial** | [core-tests/base/result](https://github.com/verum-lang/verum/tree/main/core-tests/base/result) — **task #4 §A + §C CLOSED 2026-05-15**: 5/5 test files green under interpreter (unit / property / integration / try_block / try_protocol). `RetryOptions.exponential` + `.linear` static builders added (`core/base/retry.vr`). Turbofish syntax in property_test rewritten to let-bound annotations. **Deferred §B**: cross-type `?` from `Maybe<T>?` inside `Result<T, E>`-returning fn (and inverse) blocked by `protocol::can_convert_residual` at `crates/verum_types/src/protocol.rs:8454` performing only STRUCTURAL match — does not consult impl where-clause bounds (`E: Default`). Workaround: explicit `.ok_or(...)` / `.ok()` conversions. Fix needs coordinated bound-resolution upgrade across `can_convert_residual` + `check_from_implementation` + `try_match_type`'s bound-aware paths. |
| `ordering.vr`        | **partial** | [core-tests/base/ordering](https://github.com/verum-lang/verum/tree/main/core-tests/base/ordering) — Tier-0 dispatcher inlines `Int.cmp`, `Float.cmp` (NaN-as-Equal), `Bool.cmp` (false &lt; true), `Text.cmp`. **Tasks #8 + #9 CLOSED 2026-05-14**. **Task #8 commit `d4f65f0c4`**: canonical `resolve_arg_value` helper in `cbgr_helpers.rs` unifies all three Verum ref shapes (CBGR register-ref, heap-interior ptr, ThinRef), replacing 24+ duplicated `is_cbgr_ref / else .as_T()` sites in `method_dispatch.rs`. Eliminates "primitive intercept misses heap-interior ref" defect class — affected every `cmp`/`eq`/`ne`/`lt`/`le`/`gt`/`ge` on Int/Bool/Float/Byte/Char/Int32/UInt64 with `&xs[i]`/`&record.f` args. Pre-fix `xs[0].cmp(&xs[1])` for `xs = [5, 2]` returned `Less` instead of `Greater`. **Task #9 commit `c4ced5bb0`**: new `try_emit_display_dispatch` codegen helper routes `f"{x}"` through user-defined `implement Display for T { fn fmt(&self, f: &mut Formatter) -> Result<(), FormatError> }` instead of the Debug-style `ToString` opcode. Emits canonical Formatter wiring inline (`buf := ""; formatter := Formatter.new(&mut buf); _ := value.fmt(&mut formatter); buf`). Three detection signals (function-table direct, function-table parent-scan, type-descriptor protocol probe). Primitive types keep the inline `ToString` fast path. Works for user-defined Display impls in same module AND for stdlib impls when explicitly referenced via `.fmt()`. **Task #10 commit `a840262f9` (2026-05-14)** closed the lazy-linking gap: two new cross-module detection channels — (a) function-table parent-scan for `parent_type_name == base` entries with key ending `.fmt`, surfaces stdlib impls landed under module-qualified keys like `core.base.Ordering.fmt`; (b) TypeDescriptor → ProtocolImpl probe — `ProtocolId` is a TypeId reference, and `proto_impl.methods[0]` is the canonical Display `fmt` FunctionId.  Same multi-channel applied to `Formatter.new`.  The emit path branches on the captured `display_func_id`: static `Call` when known (fastest), `CallM` fallback otherwise.  `f"{var}"` for stdlib-typed variables (Ordering / Maybe / Result / etc.) now correctly routes through the stdlib Display impl. |
| `ops.vr`             | **partial** | [core-tests/base/ops](https://github.com/verum-lang/verum/tree/main/core-tests/base/ops) — 4/4 test files green under `--interp` (was 2/3 pre-fix). 60+ unit + property + integration + 9 regression pins (2 `@ignore`'d). Full ControlFlow<B, C> + Try + FromResidual + Never + Drop surface covered. Two `@ignore`'d pins: nested-if-else chained-Result returns wrong variant (chained-function-return type tracking — same root family as task #17/#39). Workaround: use `?` operator directly rather than hand-unrolling. |
| `protocols.vr`       | **partial** | [core-tests/base/protocols](https://github.com/verum-lang/verum/tree/main/core-tests/base/protocols) — 5/5 test files green under `--interp` (was 2/4 pre-fix). 73 unit + 52 property + integration + try-protocol-agnostic + 12 regression pins (0 `@ignore`'d). Full operator + capability protocol surface covered (Eq/Ord/Hash/Clone/Copy/Default/Debug/Display/From/Into/Add..Shr). One pre-fix defect surfaced and closed: type-name collision through bare-suffix resolution — `type Item is { id: Int }` in `test_eq_reflexivity` collided with `core.meta.contexts.Item` (unit type), `self.id` mis-resolved (task #17/#39 manifestation through type-name lookup). Workaround pinned: deliberately-unique type names. |
| `iterator.vr`        | **partial** | [core-tests/base/iterator](https://github.com/verum-lang/verum/tree/main/core-tests/base/iterator) — **task #5 §3.1 + §A + user-side §F CLOSED 2026-05-15**.  4 of 5 test files green (`basic` / `property` / `integration` / `protocol_agnostic` / new `regression_test.vr`); `unit_test.vr` ~94/120 green (26 deferred across §B Transducer methods incomplete, §C Range.reduce_with default-method gap, §D `try_fold` R generic-param drift, §E+§F method-chain/HOF closure expected-type plumbing through precompiled CoreMetadata). **Task #5 §3.1**: TWO fundamental fixes in `crates/verum_types/src/infer/expr.rs::check_expr_inner` — (1) drop `!Type::Unit` gate in bidirectional Call arm so `Ok(())` resolves correctly via expected-type bidirectional check; (2) new bidirectional Path arm so bare `None`/`Less`/`Greater` resolve against expected type, not first-registered parent. **Task #5 §A**: `unfold`/`successors`/`Continue`/`Reduced` added to `core/base/mod.vr` `.iterator.{...}` re-export. **Task #5 §F user-side**: `register_function_signature` extracts function-type bounds (`F: fn(A) -> B`) into `param_type_bounds` and attaches them to TypeScheme via `with_type_bounds`; `infer_expr_call` default path instantiates protocol+type bounds and registers via `register_type_var_type_bound`. **Task #11 CLOSED**: `PathSegment::SelfValue` canonicalised to `"Self"` (not lowercase `"self"`) in both `extract_type_name` and `extract_type_name_from_ast`; every method-call return-type read funnels through `substitute_self_in_type_name`. |
...
| `panic.vr`           | **partial** | [core-tests/base/panic](https://github.com/verum-lang/verum/tree/main/core-tests/base/panic) — 4/4 test files green under `--interp` (was 1/3 pre-fix). 95 unit + 28 property + 11 integration + 9 regression pins (4 `@ignore`'d). Full panic / abort / exit / unreachable / todo + assert family (`assert`, `assert_eq`, `assert_ne`, `assert_some`, `assert_none`, `assert_ok`, `assert_err`, `assert_approx_eq`, `assert_between`, `assert_is_sorted`, `assert_contains`, `assert_panics`) + Location 3-field record + PanicInfo 2-field record + catch_unwind / CatchResult / PanicHook surface covered. Four `@ignore`'d pins for **`Formatter.write_int` lenient-stub defect**: the integer→bytes path references `Text.from_utf8_unchecked` which was private at text.vr:455; fix already staged in ulid commit (`unsafe fn` → `public unsafe fn`), activates on next precompiled-stdlib refresh. Every `loc.fmt_debug(&mut formatter)` / `Display.fmt` call eventually hits write_int and panics until then. |
| `env.vr`             | **partial** | [core-tests/base/env](https://github.com/verum-lang/verum/tree/main/core-tests/base/env) — 4/4 test files green under `--interp` (was 1/3 pre-fix). 31 unit + property + integration + 8 regression pins (3 `@ignore`'d). Full process args + env var + exit code + std helper surface (`args` / `arg` / `args_count` / `args_os` / `var` / `var_opt` / `set_var` / `remove_var` / `VarError` 2-variant / `exit` / `exit_success` / `exit_failure` / `home_dir` / `user` / `path` / `temp_dir` / `shell` / `locale`). Three `@ignore`'d pins for **argv plumbing inconsistency**: `args_count()` returns > 0 but `arg(0)` returns None — `init_process_args` not invoked or args_count/arg(i) read from different sources. |
| `data.vr`            | **partial** | [core-tests/base/data](https://github.com/verum-lang/verum/tree/main/core-tests/base/data) — 4/4 test files green under `--interp` (was 1/3 pre-fix). 60+ unit + property + integration + 10 regression pins (2 `@ignore`'d). Full Data 7-variant ADT (Null/Bool/Int/Float/Text/Array/Object) + DataBuilder (new / set / push / build) + parse_json + to_json + path traversal surface covered. Two `@ignore`'d pins: chained DataBuilder return-value defect — `.new().set(...).set(...).build()` followed by `.len()` mis-routes (same root family as base/log §A and base/mod §A — chained-builder type tracking through `.build()` return). |
| `serde.vr`           | **partial** | [core-tests/base/serde](https://github.com/verum-lang/verum/tree/main/core-tests/base/serde) — 4/4 test files green under `--interp` (was reported as 1/3 / 2 FAIL TBD; the diagnosis was stale and the suite was already green). 5 unit + 6 property + 7 integration + 9 regression pins. Full SerdeError factory surface covered (new / unexpected_type / missing_field / unknown_field) + SerdeResult<T> alias + protocol shape (Serialize / Deserialize / Serializer / Deserializer). Backend round-trip tests in `core-tests/encoding/<backend>/` (cbor / hex / base64 / base32 / json_pointer / varint). |
| `error.vr`           | **partial** | [core-tests/base/error](https://github.com/verum-lang/verum/tree/main/core-tests/base/error) — 54/89. `StackFrame.new` / Eq / format stable; remaining failures span variant-payload drift + Display/Debug formatter dispatch via `f"{...}"`. |
| `log.vr`             | **partial** | [core-tests/base/log](https://github.com/verum-lang/verum/tree/main/core-tests/base/log) — 4/4 test files green under `--interp` (was 2/3 pre-fix). 4 unit + 10 property + 8 integration + 9 regression pins (1 `@ignore`'d for §A). Full LogLevel ADT (Trace/Debug/Info/Warn/Error with severity 0..=4, canonical UPPERCASE names, total order, Default = Info), level_*() free-fn constructors, LogRecord builders (.new / .simple / .with_*), LogRecordBuilder fluent chain, NullLogger no-op backend. One `@ignore`'d pin for **chained-builder return-value field-access defect**: `let rec = LogRecordBuilder.new().level(L).message(M).field(...).build(); rec.level.name()` mis-resolves `.level` offset (same root as `[[btree_pattern_match_ref_generic_class]]` applied to chained-builder return values). Workaround in integration_test: length-only assertions; typed-local pattern documented. |
| `coinductive.vr`     | **partial** | [core-tests/base/coinductive](https://github.com/verum-lang/verum/tree/main/core-tests/base/coinductive) — 4/4 test files green under `--interp` (was 2/3 pre-fix). `core/base/coinductive.vr` extended with idiomatic method-form constructors and accessors (`CorecursiveCall.is_guarded()` / `.new`, `ProductivityResult.is_productive()` / `.is_non_productive()` / `.unguarded_names()`, `Observation.new(label, payload)`, `ObservationTrace.new` / `.from_list` / `.len` / `.is_empty` / `.get(i)` / `.prefix(n)`) — all additive. `integration_test.vr` rewritten end-to-end (16 tests) against the actual API; `regression_test.vr` NEW with 6 pins. One `@ignore`'d pin (`§E`) for the field-access defect on `t.steps[i].payload` (same root as [[btree_pattern_match_ref_generic_class]] — field-index resolver loses Observation type through `List<Observation>[i]` subscript; workaround: typed-local binding). Method-form surface gated on next precompiled-stdlib refresh. |
| `primitives.vr`      | **partial** | [core-tests/base/primitives](https://github.com/verum-lang/verum/tree/main/core-tests/base/primitives) — `Int.checked_add` / `checked_sub` / `wrapping_*` / `saturating_*` / `to_le_bytes` etc. stable post-fix (typechecker now populates `inherent_methods` from the archive — see commits `4112766c1` + `dc64b1142`). `Int.MAX`, `Int.BITS`, `Float.BITS`, `Float.NAN`, `Float.INFINITY`, `Float.PI`, `Float.MAX`, `Float.MIN`, `Float.EPSILON`, `Float.NEG_INFINITY` constants stable post-fix (body-compiled const archive descriptor now carries `is_const = true` AND `return_type = TypeRef::Concrete(<primitive TypeId>)` — see commits `d52cd695c` + `acbdb2f32`).  **Conformance: 60/60 green under `--interp`** (was 53/60 pre-fix).  **Task §A CLOSED** — three-site codegen Cast arm + runtime width-prefix normalisation: `(N as Byte).<method>` correctly emits `byte$method` form; pre-fix `compile_method_call`'s `effective_method_name` cascade had no `ExprKind::Cast` arm and routed cast receivers to `Int.<method>` (lost u8 semantics). Affected every primitive-sized-int cast (Byte / UInt8 / UInt64 / Int32 etc.).  **Task §B CLOSED** — `test_char_from_digit` updated to canonical lowercase contract.  **Task §C CLOSED** — Unit-receiver intercept in `dispatch_primitive_method` returns canonical values for every protocol method on `()`.  Pre-fix `().cmp(&())` panicked with `method 'Once.next' not found on receiver of runtime kind ()` because the dispatcher's bare-method suffix-scan picked the `Iterator.cmp` default body (monomorphised onto `Once<T>`) over the inherent `implement Ord for ()` impl; Iterator.cmp's body executes `self.next()` → `CallM { method_id: "Once.next" }`, and Unit has no `.next`.  Architectural rule pinned: every primitive's protocol-default surface MUST be reachable through `dispatch_primitive_method`.  4 fresh regression pins (lowercase char + cast-wrap u8 + cast-checked u8 + unit-dispatch) + 2 `@ignore`'d residuals (saturating_add intrinsic-width propagation; let-bound cast `VarTypeKind::Byte` folding). |
| `glob.vr`            | **partial** | [core-tests/base/glob](https://github.com/verum-lang/verum/tree/main/core-tests/base/glob) — 4/4 test files green under `--interp` (was 2/3 pre-fix). 13 unit + 5 property + 11 integration + 8 regression pins (0 `@ignore`'d). Pre-fix integration_test used a hallucinated `GlobOptions.new().case_insensitive(true)` API and called `.matches` on a `Result<GlobPattern, _>`; rewritten end-to-end with `compile_with(pattern, case_sensitive: Bool)` and `compile(...).unwrap().matches_path(...)`. Full POSIX glob surface covered: `*` (within-segment) / `**` (cross-segment) / `?` (single-char) / `[abc]` / `[a-z]` / `[!abc]` / `\*` `\?` (escape) + case-sensitive vs case-insensitive compile. |
| `coercion.vr`        | **partial** | [core-tests/base/coercion](https://github.com/verum-lang/verum/tree/main/core-tests/base/coercion) — 1/2. IntCoercible / Indexable / RangeLike / TensorLike / BytewiseFfi marker protocols loaded from .vr; `numeric_field_coercion` blocked by field-index drift. |
| `mod.vr`             | **partial** | [core-tests/base/mod](https://github.com/verum-lang/verum/tree/main/core-tests/base/mod) — 5/5 test files green under `--interp` (was 2/4 pre-fix). 16 unit + 9 property + 13 integration + 9 prelude + 14 regression pins (2 `@ignore`'d). Prelude re-exports (Maybe / Result / Ordering / List / Map / Set / Heap), type aliases (Byte / Bytes / TextResult), VERSION / VERSION_MAJOR / _MINOR / _PATCH constants, and Eq / Ord / Clone protocol accessibility all green. Two `@ignore`'d pins: (a) `(1..=10).sum()` returns 45 instead of 55 — the `..=` inclusive range drops the upper bound (defect in either the desugar or Range.sum() default-method body). (b) `take(&mut x)` returns value correctly but doesn't reset `x` to `T::default()`. Both multi-day stdlib + VBC codegen fixes. |
...
| `string_distance.vr` | **partial** | [core-tests/base/string_distance](https://github.com/verum-lang/verum/tree/main/core-tests/base/string_distance) — 4/4 test files green under `--interp` (was 1/3 pre-fix). 27 unit + 9 property + 11 integration + 7 regression pins. Levenshtein full + bounded (Wagner-Fischer, rolling-window), Jaro and Jaro-Winkler similarity, `closest_text` / `closest_n` "did you mean ..." autocorrect API. All non-`@ignore`'d tests pass. Pre-fix every test file used a hallucinated API (`&str.chars()`, `StringDistanceOptions`, `"x".bytes()` → `Bytes`, `[].to_bytes()`, 2-arg `closest_text`); rewritten end-to-end using `Text.as_bytes()` and explicit `max_distance`. One `@ignore`'d pin (`§G`) for tuple-field access through `List<(Text, Int)>[i].0` / `.1` subscript losing element type (same root as `[[btree_pattern_match_ref_generic_class]]`; workaround: clone+destructure via typed local). |
...
| `semver.vr`          | **partial** | [core-tests/base/semver](https://github.com/verum-lang/verum/tree/main/core-tests/base/semver) — 4/4 test files green under `--interp` (was 0/3 pre-fix). 19 unit + 13 property + 8 integration + 7 regression pins (2 `@ignore`'d). Pure-data surface (5-field SemVer record, cmp ordering per RFC 2.0.0 §11, SemVerError 4-variant) fully covered via direct `mk_semver` record-literal helpers. Two `@ignore`'d defects: (a) `parse` bare-name dispatch collision (task #17/#39) — mis-routes to a `ParsedType`-returning sibling. (b) `format` lenient panic-stub via `Text.from_utf8_unchecked` private visibility (fix staged via ulid work). **AOT cascade unblock landed in this branch**: 4 new public symbols (`parse_semver`, `semver_compare`, `SemVerOrdering`, `semver_zero`) closing task #7 (the `undefined function: semver_compare in interval_contains` blocker that gates stdlib-wide AOT). Plus inherent `Text.from(&Text)` method at `core/text/text.vr` — closes the stdlib-wide `Text.from("literal")` lenient-stub class. |
| `semver_constraint.vr` | **partial** | [core-tests/base/semver_constraint](https://github.com/verum-lang/verum/tree/main/core-tests/base/semver_constraint) — 4/4 test files green under `--interp` (was 1/3 pre-fix). 24 unit + 16 property + 11 integration + 8 regression pins (2 `@ignore`'d). Pure-data surface (9-variant `SemVerConstraint` ADT — `Any` / `Empty` / `Eq` / `Gt` / `Gte` / `Lt` / `Lte` / `And` / `Or`; convenience ctors `exact` / `caret` / `tilde` / `tilde_major` / `tilde_minor` / `x_range_major` / `x_range_minor`; `and` / `or` flatten-and-absorb composition; `MatchOptions` strict vs allow-prerelease) fully covered. Two `@ignore`'d pins for `parse` bare-name dispatch defect — same root as `base/semver §A` / task #17/#39: `parse(&"^1.2.3").unwrap()` mis-routes to a `MatchAst`-returning sibling, downstream `matches_impl → cmp(v, target)` panics at `field access out of bounds: ... type='MatchAst'`. Workaround in tests: direct record-literal SemVer + sum-variant + convenience-ctor construction. |
| `retry.vr`           | **partial** | [core-tests/base/retry](https://github.com/verum-lang/verum/tree/main/core-tests/base/retry) — 4/4 test files green under `--interp` (was 1/3 pre-fix). 17 unit + 12 property + 12 integration + 8 regression pins (3 `@ignore`'d). Pure-data surface (RetryBackoff 3-variant ADT, RetryOptions<E> builder chain across `.simple` / `.exponential` / `.linear` / `.with_should_retry`, first-attempt success across all 5 retry entry points) fully covered. Three `@ignore`'d pins for the **closure mutable-state defect**: any retry closure that mutates an outer-scope counter trips `method 'next' not found on receiver of runtime kind Int ... 8 candidate(s) end with .next` (SignalStream / Args / Rev / ...) — same root class as task #17/#39 via closure monomorphisation. Multi-day VBC codegen fix. |
...
| **Protocol-default-method fallback** | Walks the receiver's `TypeDescriptor.protocols`; for each impl, tries the protocol's default body (`<ProtocolName>.<method>`). | `err.message()` on a user `MyError` that `implement ErrorProtocol` without overriding `message` |
