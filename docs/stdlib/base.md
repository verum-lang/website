...
| `log.vr`             | **partial** | [core-tests/base/log](https://github.com/verum-lang/verum/tree/main/core-tests/base/log) — the conformance suite is green under the interpreter. 4 unit + 10 property + 8 integration + 9 regression pins (1 `@ignore`'d for §A). Full LogLevel ADT (Trace/Debug/Info/Warn/Error with severity 0..=4, canonical UPPERCASE names, total order, Default = Info), level_*() free-fn constructors, LogRecord builders (.new / .simple / .with_*), LogRecordBuilder fluent chain, NullLogger no-op backend. One `@ignore`'d pin for **chained-builder return-value field-access defect**: `let rec = LogRecordBuilder.new().level(L).message(M).field(...).build(); rec.level.name()` mis-resolves `.level` offset (the same field-index root as the subscript cases below, reached through a chained builder's return value). Workaround in integration_test: length-only assertions; typed-local pattern documented. |
| `coinductive.vr`     | **partial** | [core-tests/base/coinductive](https://github.com/verum-lang/verum/tree/main/core-tests/base/coinductive) — the conformance suite is green under the interpreter. `core/base/coinductive.vr` extended with idiomatic method-form constructors and accessors (`CorecursiveCall.is_guarded()` / `.new`, `ProductivityResult.is_productive()` / `.is_non_productive()` / `.unguarded_names()`, `Observation.new(label, payload)`, `ObservationTrace.new` / `.from_list` / `.len` / `.is_empty` / `.get(i)` / `.prefix(n)`) — all additive. `integration_test.vr` rewritten end-to-end (16 tests) against the actual API; `regression_test.vr` NEW with 6 pins. One `@ignore`'d pin (`§E`) for the field-access defect on `t.steps[i].payload` (the field-index resolver loses the `Observation` type through a `List<Observation>[i]` subscript; bind a typed local first). Method-form surface gated on next precompiled-stdlib refresh. |
...
```verum
// `new` is the only associated function here; the rest are METHODS.
Shared.new(value)       s.clone()          s.downgrade() -> Weak<T>
weak.upgrade() -> Maybe<Shared<T>>
s.strong_count() / s.weak_count()
```
