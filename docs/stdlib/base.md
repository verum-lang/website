...
status: partial
status_detail: >-
  2026-07-16: majority-green under --interp with pinned residuals; the #55 failure wave (16 families: protocols 39F, panic 27F, data 13F, iterator 11F, env 4F) is triaged as pool task T0148. Roots: BASE_SPECTRUM_AUDIT.md + core-tests/INVENTORY.md.
---
...
| *(uuid, ulid, nanoid, snowflake)* | **moved** | Identifier generators now live in [`core.id`](/docs/stdlib/id). They read a wall clock and draw entropy, so they are not primitives of the language — `core.base.snowflake` claimed a Snowflake ID belongs next to `Maybe` and `Result`. |
