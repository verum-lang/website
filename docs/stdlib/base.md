...
status: partial
status_detail: 2026-06-02 full sweep of ALL 27 base submodules under `--interp` against an isolated, freshly re-baked stdlib archive (enabled by the per-target archive-isolation fix aabf08ff3 — prior sweeps were masked by concurrent builds clobbering the shared `target/precompiled-stdlib/runtime.vbca`): ~1749 ok / ~270 fail + 4 crashers (env & iterator SIGSEGV, memory SIGTRAP, log timeout). Clean submodules: nanoid, semver_constraint, serde, snowflake, string_distance, coinductive. META-FINDING (isolated probes): the per-feature codegen LOGIC is correct — Int.MIN, Text.new, and boxed >2^48 bit-ops all work in USER code; base failures are predominantly stdlib-BAKE / cross-module-resolution / archive-plumbing defects (#47 class), NOT codegen bugs. Example: the uuid 22-fail cluster is `Uuid.to_text` baked to a panic-stub because `core.text.Text.new` was an unresolved forward reference when `core.base.uuid` (a lower layer) was precompiled. Fix layers by leverage: (1) #47 cross-module bake forward-refs — large tracked architectural work, must NOT be rushed (naive stub synthesis explodes the archive 12.9→110MB+); (2) thread the `__const_val_` const marker through the test-compile core_metadata load path (Int.MIN ~15 tests); (3) the 4 crashers (env isolated to unit_test.vr, likely overlaps the unmerged runtime-codegen-fixes branch); (4) float edge cases ~50. Full per-cluster root causes: `core-tests/base/BASE_SPECTRUM_AUDIT.md`. Fixes landed this cycle: per-target archive isolation (aabf08ff3), qualified-const consumption LoadK (13f403af4), bare + qualified variant Display (bf3364e0c + 19bb51b3a), char_to_text ABI (faca36b33), ProcessExit AOT decode (8d1b5cef1). See per-module status table below.
---
