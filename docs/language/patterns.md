...
Opt in to SMT verification of boolean/arithmetic guards with
...
| `guard_verifier`  | `None`  | Optional injected `&dyn GuardVerifier`. The trait lives in `verum_types`; concrete adapter implementations live in `verum_smt`. |
...
| Field                | Default | What it gates                                       |
|----------------------|---------|-----------------------------------------------------|
| `timeout_ms`         | `100`   | Per-query timeout forwarded to the active solver adapter. |
