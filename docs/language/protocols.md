...
type Display is protocol {
    fn format(&self, f: &mut Formatter) -> Result<(), FormatError>;
};
...
Protocols with no methods communicate a fact to the type checker.

```verum
type Send is protocol { };       // safe to send across threads
type Sync is protocol { };       // safe to share across threads
type Copy is protocol { };       // trivial copy semantics
type Pod  is protocol { };       // plain-old-data (no drop, no padding)
```
