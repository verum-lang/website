...
| **`verum verify`**      | Discharges refinement + `ensures` obligations via the SMT capability router. Counterexamples surface automatically in the diagnostic when a step fails. |
...
A mechanisation-status map at `docs/theorem_index.md` (not generated
today — the file does not exist) is
