...
| `soundness::agda` | `AgdaBackend` — emits Cubical Agda (`{-# OPTIONS --cubical #-}`); per-rule `K_<n>-sound` postulates whose signatures are type-checked end-to-end. Cubical Agda is the only major prover with native CCHM cubical support, so it closes the cubical-fragment gap that Lean / Coq / Isabelle leave at the meta-theoretic level. |
