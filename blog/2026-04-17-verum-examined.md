...
> **Correction, 2026-09-07.** The `~0.93 ns` above is superseded and is left in place because it is what this post said in April. Re-run on 2026-09-05 on the very same bench: **1.61 ns** valid, **1.69 ns** generation+epoch, **1.20 ns** invalid, **1.25 ns** per check in a 100-check batch. The machine was loaded during that run, so those are floors rather than clean numbers. The paragraph's shape holds — still well under the 15 ns target, still a load, a compare and a predicted branch — but the figure does not, and a number in prose is the memory of a measurement, not a measurement.
